...
public:
  ...
  PIMType GetType () const { return m_type; }
  void SetType (PIMType type) { m_type = type; }
  ...
  void SetChecksum(uint8_t checksum){ m_checksum = checksum;}
  //\}
...
public:
  ...
  JoinPruneMessage& GetJoinPruneMessage ()
  {
    if (m_type == 0)
      {
    	m_type = PIM_JP;
  ...
  AssertMessage& GetAssertMessage ()
  {
    if (m_type == 0)
      {
    	m_type = PIM_ASSERT;
  ...
  GraftAckMessage& GetGraftAckMessage()
  {
    if (m_type == 0)
      {
  ...
  StateRefreshMessage& GetStateRefreshMessage()
  {
    if (m_type == 0)
      {
