...
{
  static TypeId tid = TypeId ("ns3::pimdm::PIMHeader")
    .SetParent<Header> ()
    .AddConstructor<PIMHeader> ()
    ;
  return tid;
}
TypeId
...
{
  Buffer::Iterator i = start;
  //std::cout << "Serialize EncodedUnicast\n";
...
{
  Buffer::Iterator i = start;
  //std::cout << "Deserialize EncodedUnicast: ";
...
{
	os<< " AddFam. = "<< (uint16_t)this->m_addressFamily<<" Enc.Type = "<< (uint16_t)this->m_encodingType<<
			" B = " << (uint16_t)this->m_B << " Reserved = "<< (uint16_t)this->m_reserved << " Z = " << (uint16_t)this->m_Z<<
			" MaskLen = " << (uint16_t)this->m_maskLength<< " GroupAddress " << this->m_groupAddress<<"\n";
}
...
{
	os<< " AddFam. = "<< (uint16_t)this->m_addressFamily<<" Enc.Type = "<< (uint16_t)this->m_encodingType<<
			" Reserved = "<< (uint16_t)this->m_reserved << " S = " << (uint16_t)this->m_S<<
			" W = " << (uint16_t)this->m_W<< " R = "<<(uint16_t)this->m_R<<
			" MaskLen = " << (uint16_t)this->m_maskLength<< " SourceAddress " << this->m_sourceAddress<<"\n";
}
...
{
  uint32_t size = 0;
  for (std::vector<HelloEntry>::const_iterator iter = this->m_optionList.begin ();
       iter != this->m_optionList.end (); iter++)
    {
...
for (std::vector<HelloEntry>::const_iterator iter = this->m_optionList.begin ();
	   iter != this->m_optionList.end (); iter++)
	{
	  ...
	  switch (he.m_optionType)
	        {
	        case HelloHoldTime:{
	        	os <<  ", HoldTime = " << he.m_optionValue.holdTime.m_holdTime.GetSeconds()<<"\n";
	      	  break;
	        }
	        case LANPruneDelay:{
	        ...
	        case GenerationID:{
			  NS_ASSERT (he.m_optionLength % 4 == 0);
			  os<<" Gen.ID = "<<he.m_optionValue.generationID.m_generatioID<<"\n";
			  break;
	        }
	        case StateRefreshCapable:{
				NS_ASSERT (he.m_optionLength % 4 == 0);
				os<<" V = "<<(uint16_t)he.m_optionValue.stateRefreshCapable.m_version <<
						", Interval = "<< (uint16_t)he.m_optionValue.stateRefreshCapable.m_interval
  ...
  for (std::vector<HelloEntry>::const_iterator iter = this->m_optionList.begin ();
      ...
      switch (he.m_optionType)
	    ...
	    case LANPruneDelay:{
			...
			i.WriteHtonU16(he.m_optionValue.lanPruneDelay.m_T<<15|he.m_optionValue.lanPruneDelay.m_propagationDelay.GetMilliSeconds());
			i.WriteHtonU16(he.m_optionValue.lanPruneDelay.m_overrideInterval.GetMilliSeconds());
			break;
	    }
		case GenerationID:{
			NS_ASSERT (he.m_optionLength == PIM_DM_HELLO_GENERATIONID);
			i.WriteHtonU16(PIM_DM_HELLO_GENERATIONID);
			i.WriteHtonU32(he.m_optionValue.generationID.m_generatioID);
			break;
		}
		case StateRefreshCapable:{
	    	NS_ASSERT (he.m_optionLength == PIM_DM_HELLO_STATEREFRESH);
			i.WriteHtonU16(PIM_DM_HELLO_STATEREFRESH);
			i.WriteU8(he.m_optionValue.stateRefreshCapable.m_version);
			i.WriteU8(he.m_optionValue.stateRefreshCapable.m_interval);
  ...
  while (helloSizeLeft)
    {
      ...
      switch (he.m_optionType){
		  ...
		  case GenerationID:{
			  NS_ASSERT (he.m_optionLength == PIM_DM_HELLO_GENERATIONID);
			  he.m_optionValue.generationID.m_generatioID = i.ReadNtohU32();
			  helloSizeLeft-=PIM_DM_HELLO_GENERATIONID;
			  break;
		  }
		  case StateRefreshCapable:{
			  NS_ASSERT (he.m_optionLength == PIM_DM_HELLO_STATEREFRESH);
			  he.m_optionValue.stateRefreshCapable.m_version = i.ReadU8();
			  he.m_optionValue.stateRefreshCapable.m_interval = i.ReadU8();
			  he.m_optionValue.stateRefreshCapable.m_reserved = i.ReadNtohU16();
...
PIMHeader::AssertMessage::Print (std::ostream &os) const
{
    ...
    os <<  "Dest="<<m_destination<<" R = " << (uint16_t)this->m_R << " Metric Pref. = " << this->m_metricPreference<<
...
PIMHeader::StateRefreshMessage::Print (std::ostream &os) const
{
    ...
    os <<  " R = " << (uint16_t)this->m_R << " Metric Pref. = " << this->m_metricPreference<<
		  " Metric = " << this->m_metric<<" MaskLen = "<< (uint16_t)this->m_maskLength<<" TTL = "<<
		  (uint16_t)this->m_ttl<<" P = "<<(uint16_t)this->m_P<<" N = "<< (uint16_t)this->m_N <<" O = "<<
		  (uint16_t)this->m_O<<" Reserved = " <<(uint16_t)this->m_reserved<<" Interval = "<<(uint16_t)this->m_interval<<"\n";
}
