...
namespace ns3
{
  namespace pimdm
  {
    ...
    MulticastRoutingProtocol::MulticastRoutingProtocol () :
        m_mainInterface(0), m_mainAddress(Ipv4Address()), m_hostInterface(0), m_hostAddress(Ipv4Address()),
        m_helloHoldTime(0), m_generationID(0), m_startTime(0), m_stopTx(false), m_routingTableAssociation(0), m_ipv4(0),
        m_identification(0), m_routingProtocol(0), m_lo(0), m_rpfChecker(Timer::CANCEL_ON_DESTROY)
    {
  ...
  return tid;
    ...
    MulticastRoutingProtocol::registerMember (Ipv4Address source, Ipv4Address group, uint32_t interface)
    {
      ...
      if (m_LocalReceiver.find(sgp) == m_LocalReceiver.end())
        { //add a new receiver on a specific (source,group) on a given interface
      ...
      if (m_LocalReceiver.find(sgp)->second.find(interface) == m_LocalReceiver.find(sgp)->second.end() && interface > 0
          && interface < m_ipv4->GetNInterfaces())
        {
    ...
    MulticastRoutingProtocol::unregisterMember (Ipv4Address source, Ipv4Address group, uint32_t interface)
    {
      ...
      if (m_LocalReceiver.find(sgp) != m_LocalReceiver.end() && m_LocalReceiver.find(sgp)->second.empty())
        ...
        NS_LOG_DEBUG("No clients on interface " << interface<< " for ("<<source<<","<<group<<")");
      UpstreamStateMachine(sgp);
    }
    ...
    MulticastRoutingProtocol::register_SG (std::string csv)
    {
      ...
      if (group == Ipv4Address(ALL_PIM_ROUTERS4) || sources > 1)
        return;	//Socket already registered for this group
...
/// \param ifaceEntry			InterfaceSpecific Entry for the multicast group.
    bool
    MulticastRoutingProtocol::Lookup (const Ipv4Address group, const Ipv4Address source,
                                      RoutingMulticastTable &outEntry, MulticastEntry &me) const
    {
...
///
    bool
    MulticastRoutingProtocol::UpdateEntry (Ipv4Address const group, Ipv4Address const source, Ipv4Address const next,
                                           uint32_t const interface)
    {
...
///
    void
    MulticastRoutingProtocol::AddEntry (const Ipv4Address group, const Ipv4Address source, const Ipv4Address next,
...
///
    Ptr<Ipv4Route>
    MulticastRoutingProtocol::RouteOutput (Ptr<Packet> p, const Ipv4Header &header, Ptr<NetDevice> oif,
                                           Socket::SocketErrno &sockerr)
    {
      ...
      if (header.GetDestination().IsMulticast())
        {  //entry in the routing table found
    ...
    bool
    MulticastRoutingProtocol::RouteInput (Ptr<const Packet> p, const Ipv4Header &header, Ptr<const NetDevice> idev,
...
///
    ...
    MulticastRoutingProtocol::PrintRoutingTable (Ptr<OutputStreamWrapper> stream) const
    {
      ...
      *os << "Group\t Source\t NextHop\t Interface\n";
      ...
      for (std::map<Ipv4Address, RoutingMulticastTable>::const_iterator iter = m_mrib.begin(); iter != m_mrib.end();
          iter++)
        {
          *os << iter->first << "\t\n";
    ...
    void
    MulticastRoutingProtocol::DoStart ()
    {
      if (m_generationID == 0)
...
//	   be reset except when it expires.
    ...
    bool
    MulticastRoutingProtocol::IsDownstream (uint32_t interface, Ipv4Address neighbor, Ipv4Address source,
                                            Ipv4Address group)
    {
    ...
    bool
    MulticastRoutingProtocol::IsUpstream (uint32_t interface, Ipv4Address neighbor, Ipv4Address source,
                                          Ipv4Address group)
    {
    ...
    void
    MulticastRoutingProtocol::CreateMulticastGroupEntry (PIMHeader::MulticastGroupEntry &m_entry,
    ...
    MulticastRoutingProtocol::AddMulticastGroupEntry (PIMHeader &msg, PIMHeader::MulticastGroupEntry &entry)
    {
      ...
      switch (type)
        {
        case PIM_JP:
          msg.GetJoinPruneMessage().m_joinPruneMessage.m_numGroups = 1
              + msg.GetJoinPruneMessage().m_joinPruneMessage.m_numGroups;
          msg.GetJoinPruneMessage().m_multicastGroups.push_back(entry);
          break;
        case PIM_GRAFT:
          msg.GetGraftMessage().m_joinPruneMessage.m_numGroups = 1
              + msg.GetGraftMessage().m_joinPruneMessage.m_numGroups;
          msg.GetGraftMessage().m_multicastGroups.push_back(entry);
          break;
        case PIM_GRAFT_ACK:
          msg.GetGraftAckMessage().m_joinPruneMessage.m_numGroups = 1
              + msg.GetGraftAckMessage().m_joinPruneMessage.m_numGroups;
          msg.GetGraftAckMessage().m_multicastGroups.push_back(entry);
          break;
        }
    }
    void
    MulticastRoutingProtocol::AddMulticastGroupSourceJoin (PIMHeader::MulticastGroupEntry &m_entry,
                                                           PIMHeader::EncodedSource source)
    {
      NS_LOG_FUNCTION(this);
    ...
    void
    MulticastRoutingProtocol::AddMulticastGroupSourcePrune (PIMHeader::MulticastGroupEntry &m_entry,
                                                            PIMHeader::EncodedSource source)
    {
      NS_LOG_FUNCTION(this);
    ...
    {
      PIMHeader::HelloMessage::HelloEntry holdtime =
        { PIMHeader::HelloMessage::HelloHoldTime, PIM_DM_HELLO_HOLDTIME };
      holdtime.m_optionValue.holdTime.s_holdTime = Seconds(m_helloHoldTime);
    ...
    {
      PIMHeader::HelloMessage::HelloEntry lanpd =
        { PIMHeader::HelloMessage::LANPruneDelay, PIM_DM_HELLO_LANPRUNDELAY };
      lanpd.m_optionValue.lanPruneDelay.s_T = 0;
      lanpd.m_optionValue.lanPruneDelay.s_propagationDelay = m_IfaceNeighbors.find(interface)->second.propagationDelay;
      lanpd.m_optionValue.lanPruneDelay.s_overrideInterval = m_IfaceNeighbors.find(interface)->second.overrideInterval;
    ...
    {
      PIMHeader::HelloMessage::HelloEntry genid =
        { PIMHeader::HelloMessage::GenerationID, PIM_DM_HELLO_GENERATIONID };
      genid.m_optionValue.generationID.s_generatioID = m_generationID;
    ...
    {
      PIMHeader::HelloMessage::HelloEntry staterefresh =
        { PIMHeader::HelloMessage::StateRefreshCapable, PIM_DM_HELLO_STATEREFRESH };
      staterefresh.m_optionValue.stateRefreshCapable.s_version = 1;
      staterefresh.m_optionValue.stateRefreshCapable.s_interval = (uint8_t) (m_IfaceNeighbors.find(interface)->second
          .stateRefreshInterval).GetSeconds();
      staterefresh.m_optionValue.stateRefreshCapable.s_reserved = 0;
    ...
    MulticastRoutingProtocol::SendPruneUnicast (Ipv4Address destination, SourceGroupPair &sgp)
    {
      ...
      if (wei.first == UINT_MAX || !isValidGateway(wei.second))
        {
          Simulator::Schedule(Seconds(Graft_Retry_Period), &MulticastRoutingProtocol::SendPruneUnicast, this,
              destination, sgp);
    ...
    void
    MulticastRoutingProtocol::ForgeAssertMessage (uint32_t interface, Ipv4Address destination, PIMHeader &msg,
                                                  SourceGroupPair &sgp)
    {
      NS_LOG_FUNCTION(this << interface << destination<<sgp);
      ...
      assertMessage.m_R = 0;
      assertMessage.m_metricPreference =
          (sgState == NULL) ? GetMetricPreference(interface) : sgState->AssertWinner.metricPreference;
      assertMessage.m_metric =
          (sgState == NULL) ? GetRouteMetric(interface, sgp.sourceMulticastAddr) : sgState->AssertWinner.routeMetric;
    }
    ...
    void
    MulticastRoutingProtocol::ForgeGraftMessage (uint32_t interface, PIMHeader &msg, SourceGroupPair &sgp,
    ...
    void
    MulticastRoutingProtocol::SendGraftAckBroadcast (uint32_t interface, const Ipv4Address destination,
                                                     SourceGroupPair &sgp)
    {
      NS_LOG_FUNCTION(this);
      Ptr<Packet> packet = Create<Packet>();
      ...
      // Send the packet toward the RPF(S)
      Simulator::Schedule(TransmissionDelay(), &MulticastRoutingProtocol::SendPacketPIMRoutersInterface, this, packet,
          msg, interface);
    }
    ...
    void
    MulticastRoutingProtocol::SendAssertBroadcast (uint32_t interface, const Ipv4Address destination,
                                                   SourceGroupPair &sgp)
    {
      NS_LOG_FUNCTION(this);
      PIMHeader assertR;
    ...
    void
    MulticastRoutingProtocol::SendAssertUnicast (SourceGroupPair &sgp, uint32_t interface,
                                                 const Ipv4Address destination)
    {
      ...
      Ptr<Packet> packet = Create<Packet>();
      NS_LOG_INFO ("Node " << GetLocalAddress(interface)<< " SendAssert to "<<destination << " for "<<sgp);
      Simulator::Schedule(TransmissionDelay(), &MulticastRoutingProtocol::SendPacketPIMUnicast, this, packet, assertR,
          destination);
    }

    void
    MulticastRoutingProtocol::SendAssertCancelBroadcast (uint32_t interface, const Ipv4Address destination,
                                                         SourceGroupPair &sgp)
    {
      NS_LOG_FUNCTION(this);
      PIMHeader assertR;
    ...
    void
    MulticastRoutingProtocol::UpdateAssertTimer (SourceGroupPair &sgp, uint32_t interface, Time delay,
                                                 const Ipv4Address destination)
    {
    ...
    void
    MulticastRoutingProtocol::UpdatePruneTimer (SourceGroupPair &sgp, uint32_t interface, Time delay,
                                                const Ipv4Address destination)
    {
      SourceGroupState *sgState = FindSourceGroupState(interface, destination, sgp);
    ...
    void
    MulticastRoutingProtocol::UpdatePruneDownstreamTimer (SourceGroupPair &sgp, uint32_t interface,
                                                          const Ipv4Address destination)
    {
      SourceGroupState *sgState = FindSourceGroupState(interface, destination, sgp);
    ...
    void
    MulticastRoutingProtocol::UpdatePrunePendingTimer (SourceGroupPair &sgp, uint32_t interface, Time delay,
                                                       const Ipv4Address destination)
    {
      SourceGroupState *sgState = FindSourceGroupState(interface, destination, sgp);
    ...
    void
    MulticastRoutingProtocol::UpdateAssertTimer (SourceGroupPair &sgp, uint32_t interface,
                                                 const Ipv4Address destination)
    {
    ...
    void
    MulticastRoutingProtocol::UpdateGraftTimer (SourceGroupPair &sgp, uint32_t interface, Time delay,
                                                const Ipv4Address destination)
    {
      SourceGroupState *sgState = FindSourceGroupState(interface, destination, sgp);
    ...
    void
    MulticastRoutingProtocol::UpdateOverrideTimer (SourceGroupPair &sgp, uint32_t interface, Time delay,
                                                   const Ipv4Address destination)
    {
      SourceGroupState *sgState = FindSourceGroupState(interface, destination, sgp);
      NS_ASSERT(sgState->upstream.valid);
    ...
    void
    MulticastRoutingProtocol::UpdateOverrideTimer (SourceGroupPair &sgp, uint32_t interface,
                                                   const Ipv4Address destination)
    {
      UpdateOverrideTimer(sgp, interface, Seconds(Graft_Retry_Period), destination);
    }
    void
    MulticastRoutingProtocol::UpdatePruneLimitTimer (SourceGroupPair &sgp, uint32_t interface, Time delay,
                                                     const Ipv4Address destination)
    {
      SourceGroupState *sgState = FindSourceGroupState(interface, destination, sgp);
      NS_ASSERT(sgState->upstream.valid);
    ...
    void
    MulticastRoutingProtocol::UpdatePruneLimitTimer (SourceGroupPair &sgp, uint32_t interface,
                                                     const Ipv4Address destination)
    {
      UpdatePruneLimitTimer(sgp, interface, Seconds(t_limit), destination);
    }
    void
    MulticastRoutingProtocol::UpdateSourceActiveTimer (SourceGroupPair &sgp, uint32_t interface, Time delay,
                                                       const Ipv4Address destination)
    {
      SourceGroupState *sgState = FindSourceGroupState(interface, destination, sgp);
      NS_ASSERT(sgState->upstream.valid);
    ...
    void
    MulticastRoutingProtocol::UpdateSourceActiveTimer (SourceGroupPair &sgp, uint32_t interface,
                                                       const Ipv4Address destination)
    {
      UpdateSourceActiveTimer(sgp, interface, Seconds(SourceLifetime), destination);
    }
    void
    MulticastRoutingProtocol::UpdateStateRefreshTimer (SourceGroupPair &sgp, uint32_t interface, Time delay,
                                                       const Ipv4Address destination)
    {
      SourceGroupState *sgState = FindSourceGroupState(interface, destination, sgp);
      NS_ASSERT(sgState->upstream.valid);
    ...
    void
    MulticastRoutingProtocol::ForgeStateRefresh (uint32_t interface, Ipv4Address destination, SourceGroupPair &sgp,
                                                 PIMHeader &msg)
    {
      NS_LOG_FUNCTION(this);
      SourceGroupState *sgState = FindSourceGroupState(interface, destination, sgp);
...
//	     versions of state refresh.
    ...
    void
    MulticastRoutingProtocol::SendStateRefreshMessage (uint32_t interface, Ipv4Address destination,
                                                       SourceGroupPair &sgp)
    {
      NS_LOG_FUNCTION(this);
      SourceGroupState *sgState = FindSourceGroupState(interface, destination, sgp);
    ...
    MulticastRoutingProtocol::RPFCheckAll ()
    {
      ...
      for (std::map<WiredEquivalentInterface, SourceGroupList>::iterator sgList = m_IfaceSourceGroup.begin();
          sgList != m_IfaceSourceGroup.end(); sgList++)
        {
          ...
          for (std::list<SourceGroupState>::iterator sgState = sgList->second.begin(); sgState != sgList->second.end();
              sgState++)
            {
              Ptr<Ipv4Route> route;
    ...
    MulticastRoutingProtocol::RPFCheck (SourceGroupPair sgp)
    {
      ...
      if (ret)
        {
          if (me.nextAddr == Ipv4Address::GetLoopback())
            { //now we know the RPF for the first time, just update it!
          ...
          if ((me.interface != wei.first || me.nextAddr != wei.second) && wei.first != UINT_MAX)
            { //RPF neighbor has changed
    ...
    void
    MulticastRoutingProtocol::RPF_Changes (SourceGroupPair &sgp, uint32_t oldInterface, Ipv4Address oldGateway,
                                           uint32_t newInterface, Ipv4Address newGateway)
    {
    ...
    void
    MulticastRoutingProtocol::RPF_primeChanges (SourceGroupPair &sgp, uint32_t interfaceO, Ipv4Address gatewayO,
                                                uint32_t interfaceN, Ipv4Address gatewayN)
    {
    ...
    MulticastRoutingProtocol::RecvMessage (Ptr<Socket> socket)
    {
      ...
      if ((tag || (group.IsMulticast() && group != Ipv4Address(ALL_PIM_ROUTERS4)))
          && (interface == m_mainInterface || sourcePkt))
        {
          if (tag)
    ...
    void
    MulticastRoutingProtocol::RecvPIMDM (Ptr<Packet> receivedPacket, Ipv4Address senderIfaceAddr,
                                         uint16_t senderIfacePort, uint32_t interface)
    {
      NS_LOG_FUNCTION(this);
      ...
      if (route)
        NS_LOG_DEBUG("\t Route = "<<localAddr<< " <"<<interface<<"> " <<route->GetGateway() <<" <...> "<<senderIfaceAddr);
...
//		return AskRoute(senderIfaceAddr);
    ...
    void
    MulticastRoutingProtocol::RecvPIMData (Ptr<Packet> receivedPacket, Ipv4Address senderIfaceAddr,
                                           uint16_t senderIfacePort, uint32_t interface)
    {
      NS_LOG_FUNCTION(this);
      NS_ASSERT(interface);
      ...
      if (IsUpstream(interface, sender, sgp))
        { /*sender in on the RPF towards the source*/
          switch (sgState->upstream.GraftPrune)
            {
      ...
      if (backbone)
        {
          Time delay = TransmissionDelay(10, 250, Time::US);
          ...
          Simulator::Schedule(delay, &MulticastRoutingProtocol::SendPacketHBroadcastInterface, this, fwdPacket,
              sourceHeader, m_mainInterface);
        }
      if (clients)
        {
          Time delay = TransmissionDelay(10, 250, Time::US);
...
//	}
      ...
      for (std::vector<PIMHeader::MulticastGroupEntry>::iterator mgroup = graft.m_multicastGroups.begin();
          mgroup != graft.m_multicastGroups.end(); mgroup++)
        {
          for (std::vector<PIMHeader::EncodedSource>::iterator msource = mgroup->m_joinedSourceAddrs.begin();
            ...
            {
              if (IsDownstream(interface, sender, msource->m_sourceAddress,
                  mgroup->m_multicastGroupAddr.m_groupAddress))
                {
    ...
    void
    MulticastRoutingProtocol::RecvGraftDownstream (PIMHeader::GraftMessage &graft, Ipv4Address sender,
                                                   Ipv4Address receiver, const PIMHeader::EncodedSource &source,
                                                   PIMHeader::EncodedGroup &group, uint32_t interface)
    {
    ...
    void
    MulticastRoutingProtocol::RecvGraftAck (PIMHeader::GraftAckMessage &graftAck, Ipv4Address sender,
                                            Ipv4Address receiver, uint32_t interface)
    {
      NS_LOG_FUNCTION(this<<sender<<receiver<<interface);
      ...
      if (GetLocalAddress(interface) != graftAck.m_joinPruneMessage.m_upstreamNeighborAddr.m_unicastAddress)
        return;
...
//	 These State Refresh messages SHOULD be sent out immediately after the Hello message.
      ...
      for (std::list<SourceGroupState>::iterator sgState = sgList.begin(); sgState != sgList.end(); sgState++)
        {
          if (IsDownstream(interface, neighbor, sgState->SGPair) && sgState->AssertState == Assert_Winner)
            {
              Simulator::Schedule(srm, &MulticastRoutingProtocol::SendStateRefreshMessage, this, interface, neighbor,
                  sgState->SGPair);
              break;
            }
          ...
          //     Prune Limit Timer to permit sending a prune and reestablishing a Pruned state in the upstream router.
          else if (IsUpstream(interface, neighbor, sgState->SGPair) && sgState->PruneState == Prune_Pruned)
            {
              ...
              Simulator::ScheduleNow(&MulticastRoutingProtocol::SendPruneBroadcast, this, interface, sgState->SGPair,
                  neighbor);
            }
        }
    }
    ...
    void
    MulticastRoutingProtocol::SendPacketPIMRoutersInterface (Ptr<Packet> packet, const PIMHeader &message,
                                                             uint32_t interface)
    {
      NS_LOG_FUNCTION(this);
      if (m_stopTx)
      ...
      for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator i = m_socketAddresses.begin();
        ...
        {
          if ((uint32_t) m_ipv4->GetInterfaceForDevice(i->first->GetBoundNetDevice()) == interface
              && !i->second.GetLocal().IsMulticast())
            {
    ...
    void
    MulticastRoutingProtocol::SendPacketPIMUnicast (Ptr<Packet> packet, const PIMHeader &message,
          ...
          if (local == i->second.GetLocal())
            {
              ...
              Ipv4Header ipv4Header = BuildHeader(i->second.GetLocal(), destination, PIM_IP_PROTOCOL_NUM,
                  copy->GetSize(), 1, false);
    ...
    void
    MulticastRoutingProtocol::SendPacketHBroadcastInterface (Ptr<Packet> packet, Ipv4Header &ipv4Header,
                                                             uint32_t interface)
    {
      NS_LOG_FUNCTION(this);
      if (m_stopTx)
...
//	if(sgState->SG_PLTD.IsRunning())
    ...
    void
    MulticastRoutingProtocol::NLTTimerExpire (uint32_t interface, Ipv4Address neighborIfaceAddr,
                                              Ipv4Address receivingIfaceAddr)
    {
      ...
      for (SourceGroupList::iterator sgState = sgList->begin(); sgState != sgList->end(); sgState++)
        {
          if (sgState->AssertWinner.IPAddress == neighborIfaceAddr)
            { // Find the assert winner
              switch (sgState->AssertState)
                {
                case Assert_NoInfo:
                  {
      ...
      if (!isValidGateway(gateway))
        {
          ...
          Simulator::Schedule(Seconds(Graft_Retry_Period), &MulticastRoutingProtocol::OTTimerExpire, this, sgp,
              interface, gateway);
          return AskRoute(gateway);
        }
      if (gateway.Get() != GetNextHop(sgp.sourceMulticastAddr).Get())
        return;
        ...
        case GP_AckPending:
          {
            ...
            if (ns->neighborGraftRetry[0] < ns->neighborGraftRetry[1])
              {		//increase counter retries
        ...
        case Prune_PrunePending:
          {
            ...
            if (nstatus->neighbors.size() > 1)
              {		// Prune echo
                ...
                Ptr<Packet> packet = Create<Packet>();
                Simulator::Schedule(TransmissionDelay(), &MulticastRoutingProtocol::SendPacketPIMRoutersInterface, this,
                    packet, prune, interface);
                ...
                case Originator:
                  {
                    ...
                    refresh.GetStateRefreshMessage().m_P = (
                        IsDownstream(interface, destination, sgp) && (sgState->PruneState == Prune_Pruned) ? 1 : 0);
                    Ptr<Packet> packet = Create<Packet>();
                    Simulator::Schedule(TransmissionDelay(), &MulticastRoutingProtocol::SendPacketPIMRoutersInterface,
                        this, packet, refresh, interface);
                    break;
                  }
                default:
                  {
...
//      timer is normally set to SourceLifetime (see 4.8).
    ...
    void
    MulticastRoutingProtocol::UpstreamStateMachine (SourceGroupPair &sgp)
    {
...
//			eventUpstream = Simulator::Schedule(Seconds(Graft_Retry_Period), &MulticastRoutingProtocol::UpstreamStateMachine, this, sgp);
    ...
    void
    MulticastRoutingProtocol::CouldAssertCheck (Ipv4Address source, Ipv4Address group, uint32_t interface,
                                                Ipv4Address destination, bool couldAssert)
    {
...
//	   NoInfo (NI) state.  The PrunePending Timer (PPT(S,G,I)) MUST be cancelled.
      for (std::vector<PIMHeader::MulticastGroupEntry>::iterator iter = jp.m_multicastGroups.begin();
          iter != jp.m_multicastGroups.end(); iter++)
        {				///<Section 4.4.1.
...
//			effectively transitioning immediately to the Pruned (P) state.
    void
    MulticastRoutingProtocol::RecvPrune (PIMHeader::JoinPruneMessage &jp, Ipv4Address &sender, Ipv4Address &receiver,
                                         ...
                                         PIMHeader::EncodedGroup &group)
    {
       ...
       */
      else if (IsUpstream(interface, sender, source.m_sourceAddress, group.m_groupAddress) && upstream == sender)
        { //prune echo
...
//	Finally, a Graft message is used to re-join a previously pruned branch to the delivery tree.
    void
    MulticastRoutingProtocol::RecvPruneUpstream (PIMHeader::JoinPruneMessage &jp, Ipv4Address &sender,
                                                 Ipv4Address &receiver, uint32_t &interface,
...
//	SourceGroupState *sgState = FindSourceGroupState(interface, sender, sgp, true);
        ...
        case GP_Pruned:
          {
            ...
            if (GetNextHop(source.m_sourceAddress) == sender
                && jp.m_joinPruneMessage.m_holdTime > sgState->upstream.SG_PLT.GetDelay())
              {
                sgState->upstream.SG_PLT.Cancel();
    ...
    void
    MulticastRoutingProtocol::RecvPruneDownstream (PIMHeader::JoinPruneMessage &jp, Ipv4Address &sender,
                                                   Ipv4Address &receiver, uint32_t &interface,
                                                   const PIMHeader::EncodedSource &source,
                                                   PIMHeader::EncodedGroup &group)
    {
      NS_LOG_FUNCTION(this<<sender<<receiver<<interface<<source.m_sourceAddress<< group.m_groupAddress);
      SourceGroupPair sgp(source.m_sourceAddress, group.m_groupAddress, sender);
    ...
    void
    MulticastRoutingProtocol::RecvJoin (PIMHeader::JoinPruneMessage &jp, Ipv4Address &sender, Ipv4Address &receiver,
                                        uint32_t &interface, const PIMHeader::EncodedSource &source,
                                        PIMHeader::EncodedGroup &group)
    {
      NS_LOG_FUNCTION(this<<sender<<receiver<<interface<<source.m_sourceAddress<<group.m_groupAddress);
    ...
    void
    MulticastRoutingProtocol::RecvJoinUpstream (PIMHeader::JoinPruneMessage &jp, Ipv4Address &sender,
                                                Ipv4Address &receiver, uint32_t &interface,
                                                const PIMHeader::EncodedSource &source, PIMHeader::EncodedGroup &group)
    {
      NS_LOG_FUNCTION(this<<sender<<receiver<<interface<<source.m_sourceAddress<<group.m_groupAddress);
    ...
    void // TOCHECK
    MulticastRoutingProtocol::RecvJoinDownstream (PIMHeader::JoinPruneMessage &jp, Ipv4Address &sender,
                                                  Ipv4Address &receiver, uint32_t &interface,
                                                  const PIMHeader::EncodedSource &source,
                                                  PIMHeader::EncodedGroup &group)
    {
                                          ...
                                          uint32_t interface)
    {
      ...
      bool couldAssert = CouldAssert(assert.m_sourceAddr.m_unicastAddress, assert.m_multicastGroupAddr.m_groupAddress,
          interface, sender);
        ...
        case Assert_NoInfo:
          {
            ...
            myMetric = my_assert_metric(assert.m_sourceAddr.m_unicastAddress,
                assert.m_multicastGroupAddr.m_groupAddress, interface, sender);
    ...
    void
    MulticastRoutingProtocol::RecvStateRefresh (PIMHeader::StateRefreshMessage &refresh, Ipv4Address sender,
                                                Ipv4Address receiver, uint32_t interface)
    {
      NS_LOG_FUNCTION(this<<sender<<receiver<<interface);
      ...
      for (uint32_t i = 0;
          Simulator::Now() < Seconds(m_startTime.GetSeconds() + Hello_Period) && i < m_ipv4->GetNInterfaces()
              && refresh.m_P == 1; i++)
        {
          if (IsLoopInterface(i))
            continue;
...
//			       Prune Indicator bit equals one, the router MUST set OT(S,G) to t_override seconds.
                if (refresh.m_P == 1)
                  {
                    Simulator::Schedule(TransmissionDelay(0, t_shorter), &MulticastRoutingProtocol::SetPruneState, this,
                        interface, sender, sgp, Prune_Pruned); //TODO: check this schedule
            ...
            case GP_AckPending:
              {
                ...
                if (refresh.m_P)
                  {
                    Simulator::Schedule(TransmissionDelay(0, t_shorter), &MulticastRoutingProtocol::SetPruneState, this,
                        interface, sender, sgp, Prune_Pruned);			//TODO: sure about this schedule
                    UpdateOverrideTimer(sgp, interface, Seconds(t_override(interface)), gateway);
                  }
                if (refresh.m_P == 0)
                  {
        ...
        case Assert_NoInfo:
          {
            ...
            //   to interface I, store its own address and metric as the Assert Winner,
            //   and set the Assert Timer (AT(S, G, I)) to Assert_Time.
            struct AssertMetric received(refresh.m_metricPreference, refresh.m_metric,
                refresh.m_originatorAddr.m_unicastAddress);
            if (my_assert_metric(refresh.m_sourceAddr.m_unicastAddress, refresh.m_multicastGroupAddr.m_groupAddress,
                interface, sender) > received
                && CouldAssert(refresh.m_sourceAddr.m_unicastAddress, refresh.m_multicastGroupAddr.m_groupAddress,
                    interface, sender))
              {
                sgState->AssertState = Assert_Winner;
                UpdateAssertWinner(sgState, interface);
            ...
            else
              {
                ...
                //	received State Refresh Interval.
                sgState->AssertState = Assert_Loser;
                UpdateAssertWinner(sgState, refresh.m_metricPreference, refresh.m_metric,
                    refresh.m_originatorAddr.m_unicastAddress);
                UpdateAssertTimer(sgp, interface, Seconds(3 * RefreshInterval), sender);
                if (CouldAssert(refresh.m_sourceAddr.m_unicastAddress, refresh.m_multicastGroupAddr.m_groupAddress,
                    interface, sender))
                  {
                    //If CouldAssert(S, G, I) == TRUE,
                    //	the router MUST also multicast a Prune(S, G) to the Assert winner
...
//				sgState->SG_AT.Schedule();
    ...
    void
    MulticastRoutingProtocol::ForwardingStateRefresh (PIMHeader::StateRefreshMessage &refresh, Ipv4Address sender,
                                                      Ipv4Address receiver)
    {
      ...
      if (iif == UINT_MAX
          || IsDownstream(iif, sender, refresh.m_sourceAddr.m_unicastAddress,
              refresh.m_multicastGroupAddr.m_groupAddress))
        return;
      ...
      for (std::set<WiredEquivalentInterface>::iterator i_nbrs = nbrs.begin(); i_nbrs != nbrs.end(); i_nbrs++)
        {
          ...
          if (lost_assert(refresh.m_sourceAddr.m_unicastAddress, refresh.m_multicastGroupAddr.m_groupAddress,
              i_nbrs->first, i_nbrs->second))
          ...
          std::set<WiredEquivalentInterface> prunez = prunes(refresh.m_sourceAddr.m_unicastAddress,
              refresh.m_multicastGroupAddr.m_groupAddress);
          ...
          if (prunez.find(match) != prunez.end())
            {
              ...
              if (StateRefreshCapable(i_nbrs->first))
                {
                  SourceGroupPair sgp(refresh.m_sourceAddr.m_unicastAddress,
                      refresh.m_multicastGroupAddr.m_groupAddress, sender);
            ...
            {			// set Prune Indicator bit of SRMP' to 0;
              SRMP.m_P = 0;
            }
          ...
          Ptr<Packet> packet;
          Simulator::Schedule(TransmissionDelay(), &MulticastRoutingProtocol::SendPacketPIMRoutersInterface, this,
              packet, refreshFRW, i_nbrs->first);
        }
    }
                                         ...
                                         uint32_t interface)
    {
      ...
      if (!ns)
        {		// Hello message received from a new neighbor
          ...
          if (!sgl)
            InsertSourceGroupList(interface, sender);
        }
      while (entry < hello.m_optionList.size())
        {
          switch (hello.m_optionList[entry].m_optionType)
            {
            case PIMHeader::HelloMessage::HelloHoldTime:
              {		///< Sec. 4.3.2.
                Time value = hello.m_optionList[entry].m_optionValue.holdTime.s_holdTime;
                // NLT is running, we should reset it to the new holdtime
                switch ((uint16_t) value.GetSeconds())
                  {		///< Sec. 4.3.3.
            ...
            case PIMHeader::HelloMessage::GenerationID:
              {		// Section 4.3.4.
                /// First time node receives neighbor's generation ID
                if (!ns->neighborGenerationID)				//its value is always greater than zero
                  ns->neighborGenerationID = hello.m_optionList[entry].m_optionValue.generationID.s_generatioID;
                /// Generation ID changed
                else if (ns->neighborGenerationID != hello.m_optionList[entry].m_optionValue.generationID.s_generatioID)
                  {				///< Sec. 4.3.4.
                ...
                if (nStatus->LANDelayEnabled)
                  {
                    ...
                    ns->neighborT = hello.m_optionList[entry].m_optionValue.lanPruneDelay.s_T;
                    if (ns->neighborPropagationDelay
                        != Time(hello.m_optionList[entry].m_optionValue.lanPruneDelay.s_propagationDelay))
                      {
                        ...
                        ns->neighborPropagationDelay = Time(
                            hello.m_optionList[entry].m_optionValue.lanPruneDelay.s_propagationDelay);
                        nStatus->propagationDelay = Max(ns->neighborPropagationDelay, nStatus->propagationDelay);
                      }
                    if (ns->neighborOverrideInterval
                        != Time(hello.m_optionList[entry].m_optionValue.lanPruneDelay.s_overrideInterval))
                      {
                        ...
                        ns->neighborOverrideInterval = Time(
                            hello.m_optionList[entry].m_optionValue.lanPruneDelay.s_overrideInterval);
            ...
            case PIMHeader::HelloMessage::StateRefreshCapable:
              {
                ns->neighborVersion = hello.m_optionList[entry].m_optionValue.stateRefreshCapable.s_version;
                ns->neighborInterval = hello.m_optionList[entry].m_optionValue.stateRefreshCapable.s_interval;
                break;
              }
            default:
              {
    ...
    {
        bool
        operator() (const NeighborState & ns) const
        {
    ...
    void
    MulticastRoutingProtocol::UpdateAssertWinner (SourceGroupState *sgState, uint32_t metricP, uint32_t routeP,
                                                  Ipv4Address winner)
    {
      NS_LOG_FUNCTION(this);
    ...
    void
    MulticastRoutingProtocol::InsertSourceGroupState (uint32_t interface, Ipv4Address neighbor, SourceGroupPair sgp)
    {
    ...
    SourceGroupState*
    MulticastRoutingProtocol::FindSourceGroupState (uint32_t interface, Ipv4Address neighbor,
                                                    const SourceGroupPair &sgp)
    {
      NS_LOG_FUNCTION(this<<interface<<neighbor<<sgp.sourceMulticastAddr<<sgp.groupMulticastAddr);
    ...
    SourceGroupState*
    MulticastRoutingProtocol::FindSourceGroupState (uint32_t interface, Ipv4Address neighbor,
                                                    const SourceGroupPair &sgp, bool add)
    {
      NS_LOG_FUNCTION(this<<interface<<neighbor<<sgp.sourceMulticastAddr<<sgp.groupMulticastAddr);
      if (add)
    ...
    SourceGroupState*
    MulticastRoutingProtocol::FindSourceGroupState (uint32_t interface, Ipv4Address neighbor, const Ipv4Address source,
    ...
    void
    MulticastRoutingProtocol::EraseSourceGroupState (uint32_t interface, Ipv4Address neighbor,
                                                     const SourceGroupPair &sgp)
    {
    ...
    void
    MulticastRoutingProtocol::EraseSourceGroupState (uint32_t interface, Ipv4Address neighbor, const Ipv4Address source,
                                                     const Ipv4Address group)
    {
      SourceGroupList *sgl = FindSourceGroupList(interface, neighbor);
      NS_ASSERT(sgl);
    ...
    SourceGroupList*
    MulticastRoutingProtocol::FindSourceGroupList (uint32_t interface, Ipv4Address neighbor)
    {
      NS_LOG_FUNCTION(this<<interface<<neighbor);
      std::map<WiredEquivalentInterface, SourceGroupList>::iterator iter = m_IfaceSourceGroup.find(
          WiredEquivalentInterface(interface, neighbor));
      return (iter == m_IfaceSourceGroup.end() ? NULL : &iter->second);
    }

    void
    MulticastRoutingProtocol::InsertSourceGroupList (uint32_t interface, Ipv4Address neighbor)
    {
      NS_LOG_FUNCTION(this<<interface<<neighbor);
      WiredEquivalentInterface i_n(interface, neighbor);
      std::map<WiredEquivalentInterface, SourceGroupList>::iterator iter = m_IfaceSourceGroup.find(i_n);
    ...
    void
    MulticastRoutingProtocol::EraseSourceGroupList (uint32_t interface, Ipv4Address neighbor)
    {
      NS_LOG_FUNCTION(this<<interface<<neighbor);
          ...
          if (sgl->first.first == interface && sgl->first.second == neighbor)
            {
              for (std::list<SourceGroupState>::iterator sgs = sgl->second.begin(); sgs != sgl->second.end(); sgs++)
                {
                  EraseSourceGroupState(interface, neighbor, sgs->SGPair.sourceMulticastAddr,
                      sgs->SGPair.groupMulticastAddr);
                }
    ...
    NeighborhoodStatus*
    MulticastRoutingProtocol::FindNeighborhoodStatus (uint32_t interface)
    {
    ...
    void
    MulticastRoutingProtocol::InsertNeighborhoodStatus (const uint32_t interface)
    {
    ...
    void
    MulticastRoutingProtocol::EraseNeighborhoodStatus (const uint32_t interface)
    {
      std::map<uint32_t, NeighborhoodStatus>::iterator iter = m_IfaceNeighbors.find(interface);
    ...
    NeighborState*
    MulticastRoutingProtocol::FindNeighborState (uint32_t interface, const NeighborState ns)
    {
      return FindNeighborState(interface, ns.neighborIfaceAddr, ns.receivingIfaceAddr);
    }

    NeighborState*
    MulticastRoutingProtocol::FindNeighborState (uint32_t interface, const Ipv4Address neighbor,
    ...
    NeighborState*
    MulticastRoutingProtocol::FindNeighborState (uint32_t interface, const NeighborState ns, bool append)
    {
      if (append)
        {
          InsertNeighborhoodStatus(interface);
    ...
    void
    MulticastRoutingProtocol::InsertNeighborState (uint32_t interface, const Ipv4Address neighbor,
                                                   const Ipv4Address local)
    {
    ...
    void
    MulticastRoutingProtocol::EraseNeighborState (uint32_t interface, const NeighborState &ns)
    {
      ...
      if (nstatus)
        nstatus->neighbors.remove(*nso);
    }

    void
    MulticastRoutingProtocol::SetPropagationDelay (uint32_t interface, Time delay)
    {
      FindNeighborhoodStatus(interface)->propagationDelay = delay;
    }

    void
    MulticastRoutingProtocol::SetOverrideInterval (uint32_t interface, Time interval)
    {
      FindNeighborhoodStatus(interface)->overrideInterval = interval;
    }

    void
    MulticastRoutingProtocol::GetPrinterList (std::string string, std::set<WiredEquivalentInterface> resB)
    {
      std::stringstream ss;
      ss << string << ": ";
    ...
    void
    MulticastRoutingProtocol::SetLANDelayEnabled (uint32_t interface, bool state)
    {
    ...
    void
    MulticastRoutingProtocol::EnablePimInterface (uint32_t interface)
    {
    ...
    void
    MulticastRoutingProtocol::DisablePimInterface (uint32_t interface)
    {
      if (m_IfacePimEnabled.find(interface) == m_IfacePimEnabled.end())
    ...
    bool
    MulticastRoutingProtocol::GetPimInterface (uint32_t interface)
    {
      if (m_IfacePimEnabled.find(interface) != m_IfacePimEnabled.end())
    ...
    void
    MulticastRoutingProtocol::SetHelloHoldTime (uint16_t holdt)
    {
      m_helloHoldTime = holdt;
    }

    uint16_t
    MulticastRoutingProtocol::GetHelloHoldTime ()
    {
      return m_helloHoldTime;
    }

    void
    MulticastRoutingProtocol::SetGenerationID (uint32_t gid)
    {
      m_generationID = gid;
    }
    uint32_t
    MulticastRoutingProtocol::GetGenerationID ()
    {
      return m_generationID;
    }

    void
    MulticastRoutingProtocol::SetStopTx ()
    {
      m_stopTx = true;
    }

    void
    MulticastRoutingProtocol::UnsetStopTx ()
    {
      m_stopTx = false;
    }

    bool
    MulticastRoutingProtocol::GetStopTx ()
    {
      return m_stopTx;
    }

    bool
    MulticastRoutingProtocol::GetMulticastGroup (Ipv4Address group)
    {
      return (group.IsMulticast() && m_mrib.find(group) != m_mrib.end());
    }

    void
    MulticastRoutingProtocol::DelMulticastGroup (Ipv4Address group)
    {
...
/// Threshold (I) returns the minimum TTL that a packet must have before it can be transmitted on interface I.
    uint8_t
    MulticastRoutingProtocol::getThreshold (uint32_t interface)
    {
      return (uint8_t) 2; //TODO check what is it
    }

    void
    MulticastRoutingProtocol::Tokenize (const std::string& str, std::vector<std::string>& tokens,
...
// Find first "non-delimiter".
    ...
    Ipv4Header
    MulticastRoutingProtocol::BuildHeader (Ipv4Address source, Ipv4Address destination, uint8_t protocol,
...
/// largest value on the LAN. Otherwise, the Override_Interval (OI (I)) MUST be set to 2.5 seconds.
    double
    MulticastRoutingProtocol::t_short (uint32_t interface)
    {
      return t_override(interface);
    }

    double
    MulticastRoutingProtocol::t_override (uint32_t interface)
    {
      return UniformVariable().GetValue(0, OverrideInterval(interface));
    }

    double
    MulticastRoutingProtocol::OverrideInterval (uint32_t interface)
    {
      NeighborhoodStatus *ns = FindNeighborhoodStatus(interface);
      bool LPDO = true;
    ...
    MulticastRoutingProtocol::ForgeEncodedUnicast (Ipv4Address unicast)
    {
      PIMHeader::EncodedUnicast egroup =
        { AF_IPv4, PIM_NATIVE_ENCODING, unicast };
      return egroup;
    }

    PIMHeader::EncodedGroup
    MulticastRoutingProtocol::ForgeEncodedGroup (Ipv4Address group)
    {
      PIMHeader::EncodedGroup egroup =
        { AF_IPv4, PIM_NATIVE_ENCODING, 0, 0, 0, sizeof(Ipv4Address), group };
      return egroup;
    }

    PIMHeader::EncodedSource
    MulticastRoutingProtocol::ForgeEncodedSource (Ipv4Address source)
    {
      PIMHeader::EncodedSource esource =
        { AF_IPv4, PIM_NATIVE_ENCODING, 0, 0, 0, 0, sizeof(Ipv4Address), source };
      return esource;
    }
...
//  Assert Loser" state.
    Ipv4Address
    MulticastRoutingProtocol::RPF_prime (Ipv4Address source, Ipv4Address group)
    {
    ...
    bool
    MulticastRoutingProtocol::I_Am_Assert_loser (Ipv4Address source, Ipv4Address group, uint32_t interface,
                                                 Ipv4Address nexthop)
    {
    ...
    bool
    MulticastRoutingProtocol::CouldAssert (Ipv4Address source, Ipv4Address group, uint32_t interface,
    ...
    struct AssertMetric
    MulticastRoutingProtocol::spt_assert_metric (Ipv4Address source, uint32_t interface)
    {
      struct AssertMetric assertMetric(GetMetricPreference(interface), GetRouteMetric(interface, source),
          GetLocalAddress(interface));
      return assertMetric;
    }

    struct AssertMetric
    MulticastRoutingProtocol::infinite_assert_metric ()
    {
    ...
    struct AssertMetric
    MulticastRoutingProtocol::my_assert_metric (Ipv4Address source, Ipv4Address group, uint32_t interface,
                                                Ipv4Address neighbor)
    {
...
//	is less than the configured RefreshLimitInterval.
    bool
    MulticastRoutingProtocol::StateRefreshRateLimit (Ipv4Address source, Ipv4Address group)
    {
      WiredEquivalentInterface wei = RPF_interface(source);
    ...
    bool
    MulticastRoutingProtocol::StateRefreshCapable (uint32_t interface)
    {
      return FindNeighborhoodStatus(interface)->stateRefreshCapable;
    }

    void
    MulticastRoutingProtocol::SetPruneState (uint32_t interface, Ipv4Address neighbor, SourceGroupPair sgp,
                                             PruneState state)
    {
...
/// \param group Multicast group IPv4 address
    ...
    MulticastRoutingProtocol::olist (Ipv4Address source, Ipv4Address group)
    {
    ...
    void
    MulticastRoutingProtocol::AskRoute (Ipv4Address destination)
    {
    ...
    void
    MulticastRoutingProtocol::AskRoutez (Ipv4Address destination)
    {
      //TODO: We don't know the next hop towards the source: first node finds it, then it relies packets.
...
/// \return True if there are receivers interested in, false otherwise.
    bool
    MulticastRoutingProtocol::GetLocalReceiver (SourceGroupPair sgp)
    {
...
/// \return True if there are receivers on the particular interface that are interested in, false otherwise.
    bool
    MulticastRoutingProtocol::GetLocalReceiverInterface (SourceGroupPair sgp, uint32_t interface)
    {
      return GetLocalReceiver(sgp)
     ...
     */
    bool
    MulticastRoutingProtocol::local_receiver_include (Ipv4Address source, Ipv4Address group, uint32_t interface)
    {
...
//  pim_include (S,G) = {all interfaces I such that: local_receiver_include (S,G,I)}
    std::set<WiredEquivalentInterface>
    MulticastRoutingProtocol::pim_include (Ipv4Address source, Ipv4Address group)
    {
...
//   joins initiated on or by the router.
    bool
    MulticastRoutingProtocol::seek_traffic_from (Ipv4Address source, Ipv4Address group, uint32_t interface)
    {
      Ipv4Address nexthop = RPF_interface(source).second;
...
// but none of the local members seek to receive traffic from S.
    bool
    MulticastRoutingProtocol::local_receiver_exclude (Ipv4Address source, Ipv4Address group, uint32_t interface)
    {
...
///  pim_exclude (S,G) = {all interfaces I such that: local_receiver_exclude (S,G,I)}
    std::set<WiredEquivalentInterface>
    MulticastRoutingProtocol::pim_exclude (Ipv4Address source, Ipv4Address group)
    {
    ...
    inline bool
    MulticastRoutingProtocol::IsLoopInterface (uint32_t interface)
    {
     ...
     */
    std::set<WiredEquivalentInterface>
    MulticastRoutingProtocol::pim_nbrs (void)
    {
      std::set<WiredEquivalentInterface> pimNbrs;
      for (uint32_t i = 0; i < m_ipv4->GetNInterfaces(); i++)
        {
          if (IsLoopInterface(i) || m_IfaceNeighbors.find(i) == m_IfaceNeighbors.end())
            continue;
     ...
     */
    std::set<WiredEquivalentInterface>
    MulticastRoutingProtocol::prunes (Ipv4Address source, Ipv4Address group)
    {
     ...
     */
    std::set<WiredEquivalentInterface>
    MulticastRoutingProtocol::lost_assert (Ipv4Address source, Ipv4Address group)
    {
     ...
     */
    bool
    MulticastRoutingProtocol::lost_assert (Ipv4Address source, Ipv4Address group, uint32_t interface,
                                           Ipv4Address neighbor)
    {
...
//AssertWinner (S,G,I) defaults to NULL -> ANY
    Ipv4Address
    MulticastRoutingProtocol::AssertWinner (Ipv4Address source, Ipv4Address group, uint32_t interface,
                                            Ipv4Address neighbor)
    {
    ...
    struct AssertMetric
    MulticastRoutingProtocol::AssertWinnerMetric (Ipv4Address source, Ipv4Address group, uint32_t interface,
                                                  Ipv4Address neighbor)
    {
      SourceGroupState *sgState = FindSourceGroupState(interface, neighbor, source, group);
    ...
    bool
    MulticastRoutingProtocol::boundary (uint32_t interface, Ipv4Address group)
    {
     ...
     */
    std::set<WiredEquivalentInterface>
    MulticastRoutingProtocol::boundary (Ipv4Address G)
    {
    ...
    std::set<WiredEquivalentInterface>
    MulticastRoutingProtocol::immediate_olist (Ipv4Address source, Ipv4Address group)
    {
      ...
      GetPrinterList(
          "pim_nbrs (-) prunes (S,G) (+) (pim_include (*,G) (-) pim_exclude (S,G) ) * (+)* pim_include (S,G)", resA);
    ...
    Time
    MulticastRoutingProtocol::TransmissionDelay ()
    {
      return TransmissionDelay(1000, 3000, Time::US);
    }

    Time
    MulticastRoutingProtocol::TransmissionDelay (double l, double u)
    {
      return TransmissionDelay(l, u, Time::S);
    }

    Time
    MulticastRoutingProtocol::TransmissionDelay (double l, double u, enum Time::Unit unit)
    {
