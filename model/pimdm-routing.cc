...
MulticastRoutingProtocol::unregister_member (std::string csv){
	...
	interface = (uint32_t)atoi(tokens.at(2).c_str());
	tokens.clear();
...
{return IsDownstream(interface, sgpair.sourceMulticastAddr, sgpair.groupMulticastAddr);}

bool
MulticastRoutingProtocol::IsDownstream (uint32_t interface, Ipv4Address source, Ipv4Address group)
{return !(IsUpstream(interface, source, group));}

bool
MulticastRoutingProtocol::IsUpstream (uint32_t interface, SourceGroupPair sgpair)
{return IsUpstream(interface, sgpair.sourceMulticastAddr, sgpair.groupMulticastAddr);}

bool
MulticastRoutingProtocol::IsUpstream (uint32_t interface, Ipv4Address source, Ipv4Address group)
{
...
MulticastRoutingProtocol::SendPruneUnicast(Ipv4Address destination, SourceGroupPair &sgp)
{
	...
	uint32_t rpf_i = RPF_interface(destination);//know the interface-nextHop
	if (rpf_i< 1){ // TODO check for AODV
		return AskRoute(destination);
	} //just send it out, it will find its path
	SourceGroupState *sgState = FindSourceGroupState(RPF_interface(destination), sgp, true);
...
void
MulticastRoutingProtocol::SendAssertInterface(SourceGroupPair &sgp, uint32_t interface)
{
	NS_LOG_FUNCTION(this);
	PIMHeader assertR;
	ForgeAssertMessage(interface, assertR, sgp);
	Ptr<Packet> packet = Create<Packet> ();
	NS_LOG_INFO ("Node " << GetLocalAddress(interface)<< " send Assert for "<<sgp);
	Simulator::Schedule(TransmissionDelay(),&MulticastRoutingProtocol::SendPacketPIMRouterInterface, this, packet, assertR, interface);
}
...
MulticastRoutingProtocol::RPFCheckAll()
{
	...
	for(std::map<uint32_t, SourceGroupList>::iterator sgList = m_IfaceSourceGroup.begin();
		...
		for(std::list<SourceGroupState>::iterator sgState = sgList->second.begin(); sgState != sgList->second.end(); sgState++){
			Ptr<Ipv4Route> route;
			NS_ASSERT(interface > 0 && interface < m_ipv4->GetNInterfaces());
...
MulticastRoutingProtocol::RPFCheck (SourceGroupPair sgp)
{
	...
	MulticastEntry me;
	uint32_t interfaceN = RPF_interface(sgp.sourceMulticastAddr);
	Ipv4Address gatewayN = interfaceN == UINT_MAX ? Ipv4Address::GetLoopback():GetRoute(sgp.sourceMulticastAddr)->GetGateway();
	bool ret = Lookup(sgp.groupMulticastAddr,sgp.sourceMulticastAddr, entry, me); // there is a entry for this group/source
	ret = ret && (interfaceN != UINT_MAX && isValidGateway(gatewayN)); // there is a valid gateway
...
//			SourceGroupState *sgState = FindSourceGroupState(interfaceN, sgp, true);///we will know the gateway later, so add the entry
		...
		if((me.interface != interfaceN || me.nextAddr != gatewayN) && interfaceN>0){//RPF neighbor has changed
			...
			ret = UpdateEntry (sgp.groupMulticastAddr, sgp.sourceMulticastAddr, gatewayN, interfaceN);//continue from here: problem is that in the second roung it
			RPF_Changes (sgp, interfaceO, gatewayO, interfaceN, gatewayN);
			RPF_primeChanges (sgp, interfaceO, gatewayO, interfaceN, gatewayN);//check interface old is right
			NS_ASSERT (ret);
			ret = true;
...
MulticastRoutingProtocol::RPF_Changes(SourceGroupPair &sgp, uint32_t oldInterface, Ipv4Address oldGateway, uint32_t newInterface, Ipv4Address newGateway)
{
	...
	bool couldAssert = CouldAssert(sgp.sourceMulticastAddr, sgp.groupMulticastAddr, oldInterface);
	CouldAssertCheck(sgp.sourceMulticastAddr, sgp.groupMulticastAddr, oldInterface, oldGateway, couldAssert);
//	SourceGroupState *sgStateO = FindSourceGroupState(oldInterface, sgp);
	if(sgp.sourceMulticastAddr == oldGateway)
		SourceNoDirectlyConnected(sgp, oldInterface, oldGateway);
	SourceGroupState *sgStateN = FindSourceGroupState(newInterface, sgp, true); // true remove before of rpf_changes
	if (!sgStateN->upstream)
		sgStateN->upstream = new UpstreamState ();
	couldAssert = CouldAssert(sgp.sourceMulticastAddr, sgp.groupMulticastAddr, newInterface);
...
MulticastRoutingProtocol::RPF_primeChanges(SourceGroupPair &sgp, uint32_t interfaceO, Ipv4Address gatewayO, uint32_t interfaceN, Ipv4Address gatewayN)
{
	...
	std::set<uint32_t> outlist = olist(sgp.sourceMulticastAddr, sgp.groupMulticastAddr);
	SourceGroupState *sgState = FindSourceGroupState(interfaceO, sgp);
...
//	delete sgState->upstream;
	...
	sgState = FindSourceGroupState(interfaceN, sgp, true); // find new RPF pair...
	...
	switch (sgState->upstream->GraftPrune){
		...
		//	unicast a Graft to the new RPF'(S), and set the GraftRetry Timer (GRT(S, G)) to Graft_Retry_Period.
				...
				if(!sgState->upstream->SG_GRT.IsRunning()){
					...
					sgState->upstream->SG_GRT.SetArguments(sgp, interfaceN);
...
void
MulticastRoutingProtocol::RecvPIMData (Ptr<Packet> receivedPacket, Ipv4Address sender, uint16_t senderIfacePort, uint32_t interface)
{
	NS_LOG_FUNCTION(this);
	NS_ASSERT(interface);
		...
		switch (sgState->AssertState){
			...
			//	The router MUST send an Assert(S, G) to interface I and set the Assert Timer (AT(S, G, I) to Assert_Time.
				if (sgState->AssertState != Assert_Winner || !sgState->SG_AT.IsRunning()){
					...
					SendAssertInterface (sgp, interface);
					UpdateAssertTimer (sgp, interface);
	...
	if(IsUpstream(interface, sgp) && sgState->PruneState != Prune_Pruned ){
...
MulticastRoutingProtocol::RecvGraft (PIMHeader::GraftMessage &graft, Ipv4Address sender, Ipv4Address receiver, uint32_t interface)
{
	...
	for(std::vector<PIMHeader::MulticastGroupEntry>::iterator mgroup = graft.m_multicastGroups.begin();
		...
		for(std::vector<PIMHeader::EncodedSource>::iterator msource = mgroup->m_joinedSourceAddrs.begin();
			...
			if(!IsUpstream(interface, msource->m_sourceAddress, mgroup->m_multicastGroupAddr.m_groupAddress)){
...
void
MulticastRoutingProtocol::SendPacketPIMRouterInterface(Ptr<Packet> packet, const PIMHeader &message, uint32_t interface)
{
  NS_LOG_FUNCTION(this);
  if (m_stopTx) return;
  // Send
...
//			  << ", Local "<<GetLocalAddress(interface)<< ", If "<<  m_ipv4->GetInterfaceForDevice (i->first->GetBoundNetDevice()));
	  if(m_ipv4->GetInterfaceForDevice (i->first->GetBoundNetDevice()) == (int32_t)interface && !i->second.GetLocal ().IsMulticast()){
...
//{
//	NS_LOG_FUNCTION(this<< interface << sgp.sourceMulticastAddr << sgp.groupMulticastAddr);
//	uint32_t interface = RPF_interface(destination);
//	SourceGroupState *sgState = FindSourceGroupState(interface, sgp, true);
//	if(sgState->upstream && sgState->upstream->SG_PLT.IsRunning()){//the prune timer is not active
//		NS_LOG_DEBUG("Limiting prune on LAN with PLT");
//	}
...
MulticastRoutingProtocol::SendPacketPIMRouterUnicast(Ptr<Packet> packet, const PIMHeader &message, Ipv4Address destination)
{
  ...
  packet->AddHeader(message);
  // Send
  //uint32_t wei = RPF_interface(destination);
  uint32_t interface = RPF_interface(destination) >0 ? RPF_interface(destination) : m_ipv4->GetInterfaceForAddress(m_mainAddress);
  ...
  for (std::map<Ptr<Socket> , Ipv4InterfaceAddress>::const_iterator i = m_socketAddresses.begin (); i != m_socketAddresses.end (); i++)
	{
	  if(local == i->second.GetLocal () ){
		...
		Ipv4Address destination = i->second.GetLocal ().GetSubnetDirectedBroadcast (i->second.GetMask ());
...
void
MulticastRoutingProtocol::SendPacketInterface(Ptr<Packet> packet, uint32_t interface)
{
  NS_LOG_FUNCTION(this);
  if(m_stopTx) return;
  Ipv4Address local = GetLocalAddress(interface);
...
void MulticastRoutingProtocol::PLTTimerExpire (SourceGroupPair &sgp, uint32_t rpf_i)
{
	NS_LOG_FUNCTION(this);
	SourceGroupState *sgState = FindSourceGroupState(rpf_i, sgp);
	if(!IsUpstream(rpf_i, sgp)) return; // interface changed
	if(sgState->PruneState == Prune_Pruned) return;
	NS_LOG_DEBUG ("PLTTimerExpire "<< sgState->PruneState<< " Sending prune to "<<rpf_i);
	SendPruneUnicast(GetNextHop(sgp.sourceMulticastAddr), sgp);
	sgState->upstream->SG_PLT.SetDelay(Seconds(t_limit));
	sgState->upstream->SG_PLT.SetFunction(&MulticastRoutingProtocol::PLTTimerExpire, this);
	sgState->upstream->SG_PLT.SetArguments(sgp, rpf_i);
	sgState->upstream->SG_PLT.Schedule();
	NS_LOG_DEBUG("Schedule PLT "<< sgState->upstream->SG_PLT.GetDelay());
}

void
MulticastRoutingProtocol::NLTTimerExpire (Ipv4Address neighborIfaceAddr, Ipv4Address receivingIfaceAddr, uint32_t interface)
{
	NS_LOG_FUNCTION(this);
...
MulticastRoutingProtocol::OTTimerExpire (SourceGroupPair &sgp, uint32_t interface)
{
	...
	if(!isValidGateway(gateway)){
		...
		Simulator::Schedule(Seconds(Graft_Retry_Period),&MulticastRoutingProtocol::OTTimerExpire, this, sgp, interface);
		return AskRoute(gateway);
	}
	if (gateway.Get() != GetNextHop(sgp.sourceMulticastAddr).Get()) return;
	sgState = FindSourceGroupState(interface, sgp, true);
	switch (sgState->upstream->GraftPrune){
		case GP_Forwarding:{
...
MulticastRoutingProtocol::GRTTimerExpire (SourceGroupPair &sgp, uint32_t interface)
{
	...
	switch (sgState->upstream->GraftPrune){
		...
		//	It is RECOMMENDED that the router retry a configured number of times before ceasing retries.
			Ipv4Address rpf_prime = GetNextHop(sgp.sourceMulticastAddr);
			NeighborState dest(rpf_prime, GetLocalAddress(interface));
...
//			if(route->GetGateway() == Ipv4Address::GetLoopback()) break; //Node lost the neighbor -> send graft guarantee to find the gw
			if(ns->neighborGraftRetry[0]<ns->neighborGraftRetry[1]){//increase counter retries
				...
				SendGraftUnicast(rpf_prime, sgp);
				if(sgState->upstream->SG_GRT.IsRunning())
					sgState->upstream->SG_GRT.Cancel();
				sgState->upstream->SG_GRT.SetDelay(Seconds(Graft_Retry_Period));
				sgState->upstream->SG_GRT.SetFunction(&MulticastRoutingProtocol::GRTTimerExpire, this);
...
{
	NS_LOG_FUNCTION(this << sgp.sourceMulticastAddr << sgp.groupMulticastAddr << interface<<GetLocalAddress(interface));
	SourceGroupState *sgState = FindSourceGroupState(interface, sgp);
	NS_ASSERT_MSG(sgState,"PPTTimerExpire sgState is null");
	switch (sgState->PruneState) {
		case Prune_NoInfo:{
			NS_LOG_INFO ("Node "<< GetLocalAddress(interface) << "  NoInfo -> NoInfo "<< interface);
			// nothing
			break;
		}
		case Prune_PrunePending:{
		...
		//	state machine on interface I MUST transition to the Pruned (P) state.
			...
			NS_LOG_INFO ("Node "<< GetLocalAddress(interface) << " Prune_PrunePending -> Prune_Pruned "<< interface );
		...
		//	Prune_Hold_Time minus J/P_Override_Interval.
			...
			//	multicast by the upstream router to a LAN, with itself as the Upstream Neighbor.
				NS_LOG_INFO ("Node "<< GetLocalAddress(interface) << "  SendPruneEcho "<< interface);
				PIMHeader prune;
		...
		case Prune_Pruned:{
			NS_LOG_INFO ("Node "<< GetLocalAddress(interface) << " is already Pruned for "<< interface);
			//nothing
			break;
		}
		default:
...
MulticastRoutingProtocol::PTTimerExpire (SourceGroupPair &sgp, uint32_t interface)
{
	...
	switch (sgState->PruneState) {
		...
		//	The router MUST evaluate any possible transitions in the Upstream(S, G) state machine.
			sgState->PruneState = Prune_NoInfo;
			NS_LOG_DEBUG ("PTTimerExpire Prune_Pruned -> Prune_NoInfo.");
			UpstreamStateMachine(sgp);
			break;
		}
		default:
	...
	for(NeighborList::iterator iter = m_IfaceNeighbors.find(interface)->second.neighbors.begin();
		...
		switch (sgState->upstream->origination) {
			...
			//	sent over I. Otherwise, the Prune-Indicator bit MUST be set to 0.
				...
				refresh.GetStateRefreshMessage().m_P = (IsDownstream(interface, sgp) && (sgState->PruneState == Prune_Pruned) ? 1 : 0);
				Ptr<Packet> packet = Create<Packet> ();
...
void MulticastRoutingProtocol::UpstreamStateMachine(SourceGroupPair &sgp){
	...
	if(rpf_i == UINT_MAX || rpf_i > m_ipv4->GetNInterfaces())
		return;
	std::set<uint32_t> list = olist(sgp.sourceMulticastAddr, sgp.groupMulticastAddr);
	olistCheck(sgp, list);
}
...
MulticastRoutingProtocol::olistEmpty(SourceGroupPair &sgp)
{
	...
	uint32_t rpf_i = RPF_interface(sgp.sourceMulticastAddr);
	SourceGroupState *sgState = FindSourceGroupState(rpf_i, sgp);//todo fix
	switch (sgState->upstream->GraftPrune){
		case GP_Forwarding:{
		...
		//	The GraftRetry Timer (GRT(S, G)) MUST be cancelled, and PLT(S, G) MUST be set to t_limit seconds.
			...
			uint32_t rpf_i= RPF_interface(sgp.sourceMulticastAddr);
			PIMHeader msg;
			ForgeJoinPruneMessage(msg, GetNextHop(sgp.sourceMulticastAddr));
			PIMHeader::MulticastGroupEntry mge;
			CreateMulticastGroupEntry(mge, ForgeEncodedGroup(sgp.groupMulticastAddr));
			AddMulticastGroupSourcePrune(mge, ForgeEncodedSource(sgp.sourceMulticastAddr));
			AddMulticastGroupEntry(msg, mge);
...
{
	uint32_t rpf_i = (uint32_t) RPF_interface(sgp.sourceMulticastAddr);
...
//	if(!isValidGateway(rpf_i)) return AskRoute(sgp.sourceMulticastAddr);//TODO FIX for AODV
	...
	switch (sgState->upstream->GraftPrune){
		case GP_Forwarding:{
			NS_LOG_INFO("Node "<< GetLocalAddress(rpf_i)<< " GP_Forwarding -> GP_Forwarding");
			//nothing
			break;
		}
		case GP_Pruned:{
        ...
        //   The Graft Retry Timer (GRT(S, G)) MUST be set to Graft_Retry_Period.
			if(GetNextHop(sgp.sourceMulticastAddr) != sgp.sourceMulticastAddr){
				sgState->upstream->SG_PLT.Cancel();
				NS_LOG_INFO("Node "<< GetLocalAddress(rpf_i) << " GP_Pruned -> GP_AckPending");
				sgState->upstream->GraftPrune = GP_AckPending;
				SendGraftUnicast(GetNextHop(sgp.sourceMulticastAddr), sgp);
				sgState->upstream->SG_GRT.Cancel();
				sgState->upstream->SG_GRT.SetDelay(Seconds(Graft_Retry_Period));
				sgState->upstream->SG_GRT.SetFunction(&MulticastRoutingProtocol::GRTTimerExpire, this);
				sgState->upstream->SG_GRT.SetArguments(sgp, rpf_i);
				sgState->upstream->SG_GRT.Schedule();
			}
			break;
		}
		case GP_AckPending:{
			NS_LOG_INFO("Node "<< GetLocalAddress(rpf_i)<< " GP_AckPending -> GP_AckPending");
			//nothing
			break;
		}
		default:{
...
{
	NS_LOG_FUNCTION(this);
	uint32_t interface = RPF_interface(sgp.sourceMulticastAddr);
	SourceGroupState *sgState = FindSourceGroupState(interface, sgp);
	switch (sgState->upstream->GraftPrune){
		case GP_Forwarding:{
			//nothing
...
//				uint32_t out_interface = iterList->first;
...
MulticastRoutingProtocol::RecvPrune (PIMHeader::JoinPruneMessage &jp, Ipv4Address &sender, Ipv4Address &receiver, uint32_t &interface, const PIMHeader::EncodedSource &source, PIMHeader::EncodedGroup &group)
{
	...
	if(IsUpstream(interface, source.m_sourceAddress, group.m_groupAddress))
	...
	switch (sgState->upstream->GraftPrune) {
		...
		//	if it is greater than the current value of the PLT(S, G).
			if(GetNextHop(source.m_sourceAddress) == sender && jp.m_joinPruneMessage.m_holdTime>sgState->upstream->SG_PLT.GetDelay()){
				if(sgState->upstream->SG_PLT.IsRunning()){
					sgState->upstream->SG_PLT.Cancel();
					NS_LOG_DEBUG("Cancel Schedule PLT "<< sgState->upstream->SG_PLT.GetDelay());
				}
...
MulticastRoutingProtocol::RecvPruneDownstream (PIMHeader::JoinPruneMessage &jp, Ipv4Address &sender, Ipv4Address &receiver, uint32_t &interface, const PIMHeader::EncodedSource &source, PIMHeader::EncodedGroup &group)
{
	...
	switch (sgState->PruneState) {
		case Prune_NoInfo:{
			...
			if(IsMyOwnAddress(jp.m_joinPruneMessage.m_upstreamNeighborAddr.m_unicastAddress)){
				...
				NS_LOG_INFO ("Node "<<GetLocalAddress(interface)<< " RecvPrune by downstream Prune_NoInfo -> Prune_PrunePending");
				NeighborhoodStatus *nstatus = FindNeighborhoodStatus(interface);
		...
		case Prune_PrunePending:{
			NS_LOG_INFO ("Node "<<GetLocalAddress(interface)<< " RecvPrune by downstream Prune_PrunePending -> Prune_PrunePending");
			break;
		}
		case Prune_Pruned:{
	...
	switch (sgState->AssertState){
		case  Assert_NoInfo:{
			NS_LOG_INFO ("Node "<<GetLocalAddress(interface)<< " RecvPrune by downstream Assert_NoInfo -> Assert_NoInfo");
			//nothing
			break;
			}
		case Assert_Winner:{
			NS_LOG_INFO ("Node "<<GetLocalAddress(interface)<< " RecvPrune by downstream Assert_Winner -> Assert_Winner");
			//nothing
			break;
			}
		case Assert_Loser:{
...
//	NeighborState *ns = FindNeighborState(interface, neighbor);
	...
	SourceGroupState *sgState = FindSourceGroupState(interface, sgp, true);
//	uint32_t rpf_i = RPF_interface(sgp.sourceMulticastAddr);
	NS_LOG_LOGIC("Source " << sgp.sourceMulticastAddr << " Group " << sgp.groupMulticastAddr << " Gateway "<< GetNextHop(sgp.sourceMulticastAddr)<< " Sender "<< sender <<" Receiver "<<receiver << " Interface "<< interface);
//	if(!isValidGateway(gateway)) return AskRoute(sgp.sourceMulticastAddr);//todo fix
	...
	for(uint32_t i = 0; Simulator::Now() < Seconds(m_startTime.GetSeconds()+Hello_Period) && i < m_ipv4->GetNInterfaces() &&  refresh.m_P == 1;i++){
		...
		for(std::map<uint32_t, SourceGroupList>::iterator iter = m_IfaceSourceGroup.begin(); iter != m_IfaceSourceGroup.end(); iter++){
//			Ipv4Address neighbor = iter->first.second;
			sgStateB = FindSourceGroupState(interface, sgp);
			if (IsUpstream(interface, sgp) && refresh.m_P){
				...
				sgStateB->upstream->SG_PLT.SetArguments(sgp, iter->first);
				sgStateB->upstream->SG_PLT.Schedule();
			}
			else if(IsDownstream(interface, sgp) && refresh.m_P){
				...
				sgStateB->SG_PT.SetArguments(sgp, iter->first);
...
//			       Prune Indicator bit equals one, the router MUST set OT(S,G) to t_override seconds.
				if(refresh.m_P == 1){
					Simulator::Schedule (TransmissionDelay(0, t_shorter), &MulticastRoutingProtocol::SetPruneState, this, interface, sgp, Prune_Pruned); //TODO: sure about this schedule
					if(!sgState->upstream->SG_OT.IsRunning()){
					sgState->upstream->SG_OT.SetDelay(Seconds(t_override(interface)));
					sgState->upstream->SG_OT.SetFunction(&MulticastRoutingProtocol::OTTimerExpire, this);
					NS_LOG_LOGIC("Setting TIMER OT "<< sgp.sourceMulticastAddr<<", "<< sgp.groupMulticastAddr<<", "<<sgp.nextMulticastAddr<<", "<<interface);
...
//	uint32_t wei = RPF_interface(refresh.m_sourceAddr.m_unicastAddress);
	uint32_t iif = RPF_interface(sender);
	if (iif == UINT_MAX || IsDownstream(iif, refresh.m_sourceAddr.m_unicastAddress, refresh.m_multicastGroupAddr.m_groupAddress ))
		return;
	...
	for(std::set<uint32_t>::iterator i_nbrs = nbrs.begin(); i_nbrs != nbrs.end() ; i_nbrs++){
		...
		if (refresh.m_ttl == 0 || (uint32_t)(refresh.m_ttl - 1) < getThreshold(*i_nbrs))
		...
		SRMP.m_metric = GetRouteMetric(*i_nbrs, refresh.m_sourceAddr.m_unicastAddress);//(refresh.m_sourceAddr.m_unicastAddress));
		// set pref of ' to preference of unicast route used to reach S;
		SRMP.m_metricPreference = GetMetricPreference(*i_nbrs);//GetReceivingInterface(refresh.m_sourceAddr.m_unicastAddress));
...
MulticastRoutingProtocol::RecvHello(PIMHeader::HelloMessage &hello, Ipv4Address sender, Ipv4Address receiver, uint32_t interface)
{
	...
	if(!ns){// Hello message received from a new neighbor
		...
		if(!sgl) InsertSourceGroupList(interface);
	}
...
SourceGroupList* MulticastRoutingProtocol::FindSourceGroupList (uint32_t interface) {
	NS_LOG_FUNCTION(this<<interface);
	std::map<uint32_t, SourceGroupList>::iterator iter = m_IfaceSourceGroup.find (uint32_t(interface));
	return (iter == m_IfaceSourceGroup.end () ? NULL : &iter->second);
}

void MulticastRoutingProtocol::InsertSourceGroupList (uint32_t interface) {
	NS_LOG_FUNCTION(this<<interface);
	std::map<uint32_t, SourceGroupList>::iterator iter = m_IfaceSourceGroup.find (uint32_t(interface));
	if (iter != m_IfaceSourceGroup.end ())
		return;
	SourceGroupList *sgl = new SourceGroupList ();
	std::pair<uint32_t , SourceGroupList> i_s (uint32_t(interface), *sgl);
	m_IfaceSourceGroup.insert (i_s);
}

void MulticastRoutingProtocol::InsertSourceGroupState (uint32_t interface, SourceGroupPair sgp) {
	NS_LOG_FUNCTION(this<<interface<<sgp.sourceMulticastAddr<<sgp.groupMulticastAddr);
	SourceGroupState *sgState = FindSourceGroupState (interface, sgp);
	NS_ASSERT(interface >0 && interface <m_ipv4->GetNInterfaces());
	if (!sgState) {
		SourceGroupState *sgs = new SourceGroupState (sgp);
		sgs->SGPair.nextMulticastAddr = GetNextHop(sgp.sourceMulticastAddr);
		sgs->LocalMembership = Local_NoInfo;
		sgs->PruneState = Prune_NoInfo;
		sgs->AssertState = Assert_NoInfo;
		SourceGroupList *sgl = new SourceGroupList ();
		std::pair<uint32_t,SourceGroupList> k_pair(interface,*sgl);
		m_IfaceSourceGroup.insert(k_pair);
		NS_ASSERT(m_IfaceSourceGroup.find(interface) != m_IfaceSourceGroup.end());
		m_IfaceSourceGroup.find(interface)->second.push_back(*sgs);
		NS_ASSERT(m_IfaceSourceGroup.find(interface)->second.size() > 0);
		sgState = FindSourceGroupState (interface, sgp);
		if(IsUpstream(interface,sgp) && !sgState->upstream){
			sgState->upstream = new UpstreamState ();
		}
		}
	}

SourceGroupState* MulticastRoutingProtocol::FindSourceGroupState (uint32_t interface, const SourceGroupPair &sgp) {
	NS_LOG_FUNCTION(this<<interface<<sgp.sourceMulticastAddr<<sgp.groupMulticastAddr);
	SourceGroupList *sgl = FindSourceGroupList (interface);
	if (!sgl)
		return NULL;
...
SourceGroupState* MulticastRoutingProtocol::FindSourceGroupState (uint32_t interface, const SourceGroupPair &sgp, bool add) {
	NS_LOG_FUNCTION(this<<interface<<sgp.sourceMulticastAddr<<sgp.groupMulticastAddr);
	if (add && !FindSourceGroupList(interface))
		InsertSourceGroupList(interface);
	if (add && !FindSourceGroupState(interface, sgp))
...
SourceGroupState* MulticastRoutingProtocol::FindSourceGroupState (uint32_t interface,  const Ipv4Address source, const Ipv4Address group) {
	NS_LOG_FUNCTION(this<<interface<<source<<group);
	SourceGroupPair *sgp =new SourceGroupPair (source, group);
	return FindSourceGroupState (interface, *sgp);
}

void MulticastRoutingProtocol::EraseSourceGroupList (uint32_t interface) {
	NS_LOG_FUNCTION(this<<interface);
	m_IfaceSourceGroup.erase (uint32_t(interface));
}

void MulticastRoutingProtocol::EraseSourceGroupState (uint32_t interface, const Ipv4Address source, const Ipv4Address group)
{
	SourceGroupList *sgl = FindSourceGroupList (interface);
	NS_ASSERT(sgl);
	SourceGroupPair sgp (source, group);
	SourceGroupState *sgs = FindSourceGroupState(interface, sgp);
	NS_ASSERT(sgs);
	sgs->SG_AT.Remove();
	sgs->SG_PPT.Remove();
	sgs->SG_PT.Remove();
...
void MulticastRoutingProtocol::InsertNeighborState(uint32_t interface, const NeighborState ns) {
	...
	NS_LOG_FUNCTION(this<<interface);
		...
		Ipv4Address neighbor = ns.neighborIfaceAddr;
		Ipv4Address local = GetLocalAddress(interface);
		...
		neighborState = FindNeighborState(interface, *neighborState);
...
void MulticastRoutingProtocol::GetPrinterList (std::string string, std::set<uint32_t > resB){
	std::stringstream ss;
	ss<<string<<": ";
...
//uint32_t MulticastRoutingProtocol::GetReceivingInterface (Ipv4Address addr){
...
/// Threshold (I) returns the minimum TTL that a packet must have before it can be transmitted on interface I.
uint32_t MulticastRoutingProtocol::getThreshold (uint32_t interface){
...
Ipv4Address MulticastRoutingProtocol::RPF_prime (Ipv4Address source, Ipv4Address group) {
	uint32_t rpf_i = RPF_interface (source);
	if (I_Am_Assert_loser (source, group, rpf_i)) {
		return AssertWinner (source, group, rpf_i);
	} else {
...
bool MulticastRoutingProtocol::I_Am_Assert_loser (Ipv4Address source, Ipv4Address group, uint32_t interface) {
...
bool MulticastRoutingProtocol::CouldAssert (Ipv4Address source, Ipv4Address group, uint32_t interface) {
	uint32_t rpf_i = RPF_interface (source);
	bool couldAssert = !(rpf_i == interface);
	return couldAssert;
}
...
struct AssertMetric MulticastRoutingProtocol::my_assert_metric (Ipv4Address source, Ipv4Address group, uint32_t interface) {
...
bool MulticastRoutingProtocol::StateRefreshRateLimit (Ipv4Address source, Ipv4Address group){
	uint32_t rpf_i = RPF_interface (source);
	return FindSourceGroupState (rpf_i, source, group)->lastStateRefresh.GetSeconds () < RefreshInterval;
}
...
void MulticastRoutingProtocol::SetPruneState (uint32_t interface, SourceGroupPair sgp, PruneState state){
	FindSourceGroupState (interface, sgp)->PruneState = state;
}
...
std::set<uint32_t > MulticastRoutingProtocol::olist (Ipv4Address source, Ipv4Address group) {
...
std::set<uint32_t > MulticastRoutingProtocol::pim_include (Ipv4Address source, Ipv4Address group) {
...
bool MulticastRoutingProtocol::seek_traffic_from (Ipv4Address source, Ipv4Address group,uint32_t interface) {
	SourceGroupState *sgs = FindSourceGroupState (interface, source, group);
...
std::set<uint32_t > MulticastRoutingProtocol::pim_exclude (Ipv4Address source, Ipv4Address group) {
 ...
 */
std::set<uint32_t > MulticastRoutingProtocol::pim_nbrs (void) {
 ...
 */
std::set<uint32_t > MulticastRoutingProtocol::prunes (Ipv4Address source, Ipv4Address group) {
	...
	for (uint32_t i = 0; i < m_ipv4->GetNInterfaces (); i++) {
		...
		for(std::map<uint32_t, SourceGroupList>::iterator iter = m_IfaceSourceGroup.begin();
			...
			if(i!=iter->first) continue;
			SourceGroupState *sgState = FindSourceGroupState (iter->first, sgp);
			if (!sgState) continue;
			if (IsDownstream(iter->first, sgp) && sgState->PruneState == Prune_Pruned) {
 ...
 *
 */
std::set<uint32_t > MulticastRoutingProtocol::lost_assert (Ipv4Address source, Ipv4Address group) {
 ...
 */
bool MulticastRoutingProtocol::lost_assert (Ipv4Address source, Ipv4Address group, uint32_t interface) {
	if(IsUpstream(interface, source, group))
		return false;
...
Ipv4Address MulticastRoutingProtocol::AssertWinner (Ipv4Address source, Ipv4Address group, uint32_t interface) {
	SourceGroupState *sgState = FindSourceGroupState (interface, source, group);
	return (sgState) ? sgState->AssertWinner.IPAddress : Ipv4Address::GetAny ();
}

struct AssertMetric MulticastRoutingProtocol::AssertWinnerMetric (Ipv4Address source, Ipv4Address group, uint32_t interface) {
	SourceGroupState *sgState = FindSourceGroupState (interface, source, group);
 ...
 */
std::set<uint32_t > MulticastRoutingProtocol::boundary (Ipv4Address G) {
...
std::set<uint32_t > MulticastRoutingProtocol::immediate_olist (Ipv4Address source, Ipv4Address group) {
	...
	/// pim_nbrs * (-)* prunes (S,G)
	std::set_difference (pim_nbrz.begin (), pim_nbrz.end (), prunez.begin (), prunez.end (), std::inserter (resA, resA.end ()));
...
//* Packets TTL
//* Fix methods consistency with params
