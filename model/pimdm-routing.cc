...
uint32_t
MulticastRoutingProtocol::GetRecevingInterface(Ipv4Address addr){
	for (uint32_t i = 0; i < m_ipv4->GetNInterfaces (); i++)
	{
		Ipv4Address current = m_ipv4->GetAddress (i, 0).GetLocal ();
		if (current == addr)
			return i;
	}
	return 0;
...
uint32_t
MulticastRoutingProtocol::GetRecevingInterface(Ipv4InterfaceAddress interface){
	for (uint32_t i = 0; i < m_ipv4->GetNInterfaces (); i++)
	{
		if (m_ipv4->GetAddress (i, 0) == interface)
			return i;
	}
	return 0;
}
...
MulticastRoutingProtocol::PLTTimerExpire (SourceGroupPair &sgp){
	...
	SendPrune(RPF_interface(sgp.sourceIfaceAddr),sgp);
	sgState->upstream->SG_PLT.Schedule();
}
		...
		// olist(S,G) -> NULL AND S NOT directly connected
       	...
       	//   send a Prune(S,G) to RPF'(S), and set PLT(S,G) to t_limit seconds.
			if(m_mrib.find(sgp.sourceIfaceAddr)->second.nextAddr != sgp.sourceIfaceAddr){
				sgState ->upstream->SGGraftPrune = GP_Pruned;
				Ipv4Address target = RPF_prime(sgp.sourceIfaceAddr,sgp.groupMulticastAddr);
				SendPrune(RPF_interface(target),sgp);
...
uint32_t iif = GetRecevingInterface(sender);
			...
			case PIMHeader::HelloMessage::GenerationID:{// Section 4.3.4.
				...
				/// Generation ID changed
					...
					for(std::list<SourceGroupState>::iterator sgElement = sgList.begin(); sgElement!=sgList.end(); sgElement++){
						...
						for (uint32_t i = 0; i < m_ipv4->GetNInterfaces (); i++){
							if(IsDownstream(i,sgElement->SGPair) && sgElement->SGAssertState == Assert_Winner){
							...
							//     Prune Limit Timer to permit sending a prune and reestablishing a Pruned state in the upstream router.
								...
								Simulator::Schedule (Seconds(0), &MulticastRoutingProtocol::SendPrune, this, interface, sgElement->SGPair);
							}
