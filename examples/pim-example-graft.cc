...
static void Unlink(Ptr<Node> n,  uint32_t interface){
//	Ptr< CsmaChannel > r1c =  n->GetDevice(interface)->GetChannel()->GetObject< CsmaChannel >();
//	r1c->Detach(interface);
//	Ptr< CsmaNetDevice > dev = r1c->GetCsmaDevice(interface);
	Config::Set("NodeList/[0-1]/DeviceList/0/$ns3::CsmaNetDevice/SendEnable", BooleanValue(false));
	Config::Set("NodeList/[0-1]/DeviceList/0/$ns3::CsmaNetDevice/ReceiveEnable", BooleanValue(false));
}
...
//	Packet::EnablePrinting ();
	...
	Simulator::Schedule(Seconds(14), &Unlink,routers.Get(1),1);
	Simulator::Run ();
	NS_LOG_INFO ("Done.");
